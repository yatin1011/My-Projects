...
def getfinancialreportingdf(ticker):
	...
	for title in titlesfinancials:
		if 'EPS (Basic)' in title.text:
			epslist.appemd([td.text for td in title.findNextSiblings(attrs = {'class': 'valueCell'}) if td.text])
	...
	df = pd.DataFrame({'eps': eps, 'epsgrowth': epsgrowth, 'netincome': netincome, 'shareholderequity': shareholderequity,'roa': roa,'longtermdebt': longtermdebt,'interestexpense': interestexpense,'ebitda': ebitda},index=range(date.today().year-5,date.today().year))
...
# This will keep ticker + gics industries & sub industries
...
def save_self_stocks_info():
		...
		dictlist.append({'value': '', 'label': 'GME Game Stop Corp SA'})
		dictlist.append({'value': '', 'label': 'AEG Aegon Insurance SA'})
		dictlist.append({'value': '', 'label': 'NTIC Northern Technologies International SA'})
		dictlist.append({'value': '', 'label': 'SQ Square SA'})
		dictlist.append({'value': '', 'label': 'Kobe steel'})
		dictlist.append({'value': '', 'label': 'Nestle'})
		dictlist.append({'value': '', 'label': 'Danone'})
		dictlist.append({'value': '', 'label': 'Tableau Software Data Visualization'})
		dictlist.append({'value': '', 'label': 'SATS'})
